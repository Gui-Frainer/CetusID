...
def load_training_images_CNN1(species_dir, soundscape_dir):
    ...
    sample_amount_2 = int(sample_amount//2)
    nondolphin_X = nondolphin_X[np.random.choice(nondolphin_X.shape[0], sample_amount_2, replace=True)]
    dolphin_X = dolphin_X[np.random.choice(dolphin_X.shape[0], sample_amount_2, replace=True)]
...
def train_model_CNN1(number_iterations, species_dir, soundscape_dir,species_dir_val,soundscape_dir_val,network_CNN1, plot_confusion_matrix, decision_val,location_models,prediction_folder):
    ...
    for experiment_id in range(0,number_iterations):
        ...
        #Adding two channels to the dataset!
        X_train = np.stack([X_train,X_train,X_train], axis=3)
        X_train = np.squeeze(X_train)
        X_val = np.stack([X_val,X_val,X_val], axis=3)
